...
function logoIcon(){return 'assets/TOTIME_logo_only.png'}
function logoWordmark(){return 'assets/TOTIME_logo_wordmark.png'}
...
function annualTotals(year=currentYear()){const a=annualMonthData(year);return a.reduce((t,m)=>{t.consuntivato+=m.consuntivato;t.fatturato+=m.fatturato;t.incassato+=m.incassato;return t},{consuntivato:0,fatturato:0,incassato:0,daIncassare:0})}
...
function annualTaxCalc(year=currentYear()){
  const ts=currentTaxSetting(year);const totalsY=annualTotals(year);totalsY.daIncassare=Math.max(0,totalsY.fatturato-totalsY.incassato);
...
async function signUpDetailed(ev){
  ...
  await sb.auth.signOut(); session=null; state.view='login'; state.message='Registrazione completata. Torna al login e accedi con le credenziali scelte.'; render();
}
...
function homeIncassiCard(){const yr=currentYear();const cur=annualTotals(yr);const prev=annualTotals(yr-1);return `<div class="card"><b>I tuoi incassi</b><div class="kpiGrid" style="margin-top:14px"><div><span><b style="color:var(--primary)">●</b> ${yr}</span><strong>${fmtEUR(cur.incassato)}</strong></div><div><span><b style="color:var(--orange)">●</b> ${yr-1}</span><strong>${fmtEUR(prev.incassato)}</strong></div></div><div class="metricLine" style="margin-top:12px">Da incassare <span class="dot">·</span> ${fmtEUR(cur.daIncassare)}</div></div>`}
function home(){const t=totals();const y=annualTotals(currentYear());return appShell(`<button class="primary cta" onclick="newEntryChoice()">+ Nuovo consuntivo</button><div class="homeTop">${monthSelector()}</div><div class="dashboardCard heroCard"><div class="kpiGrid three"><div><span>Ore consuntivate</span><strong>${fmtNum(t.hours,1)} h</strong><small>${fmtNum(t.days,1)} gg/u</small></div><div><span>Importo mese</span><strong>${fmtEUR(t.amount)}</strong><small>${monthLabel(state.month)}</small></div><div><span>Anno in corso</span><strong>${fmtEUR(y.consuntivato)}</strong><small>consuntivato</small></div></div><div class="chartWrap"><div class="chartTitle"><span>Andamento reale anno ${currentYear()}</span><span>linea fino al mese corrente</span></div>${annualChartSvg()}</div></div>${homeIncassiCard()}`)}
...
function projectionCalc(year=currentYear()){
  ...
  const currentProjected=(selectedIsCurrent&&includeCurrent&&!excluded.includes(currentIdx))?currentActual/currentDay*daysInCurrent:currentActual;
  ...
  const base=(selectedIsCurrent?closedTotal+(includeCurrent&&!excluded.includes(currentIdx)?currentProjected:currentActual)+(weightedMonthly*futureMonths):months.reduce((a,b)=>a+b,0));
...
function settings(){return appShell(`<div class="screenTitle">Configurazione</div><div class="list"><div class="row" onclick="go('clients')"><div class="roundIcon blue">👤</div><div><div class="title">Clienti</div><div class="desc">Tariffe e tipo compenso</div></div><div>›</div></div><div class="row" onclick="go('projects')"><div class="roundIcon purple">📁</div><div><div class="title">Progetti / Clienti finali</div><div class="desc">Collegati al cliente principale</div></div><div>›</div></div><div class="row" onclick="go('activities')"><div class="roundIcon green">🏷️</div><div><div class="title">Attività</div><div class="desc">PM, AMS, Gestione...</div></div><div>›</div></div><div class="row" onclick="go('expenseCategories')"><div class="roundIcon orange">✈</div><div><div class="title">Voci spesa</div><div class="desc">Rimborso KM, hotel, pranzo, parcheggio...</div></div><div>›</div></div><div class="row" onclick="go('invoiceTemplates')"><div class="roundIcon blue">🧾</div><div><div class="title">Template fattura / Fiscozen</div><div class="desc">Personalizza descrizioni da copiare</div></div><div>›</div></div><div class="row" onclick="go('tax')"><div class="roundIcon orange">%</div><div><div class="title">Fiscalità</div><div class="desc">Forfettario, ATECO, tasse e rivalsa</div></div><div>›</div></div><div class="row" onclick="go('appearance')"><div class="roundIcon gray">◐</div><div><div class="title">Aspetto / Tema</div><div class="desc">Chiaro giorno o scuro sera</div></div><div>›</div></div><div class="row" onclick="go('exportTimesheet')"><div class="roundIcon green">⬇</div><div><div class="title">Export Timesheet Excel</div><div class="desc">Scarica il dettaglio mensile per cliente</div></div><div>›</div></div><div class="row"><div class="roundIcon gray">☁</div><div><div class="title">Database</div><div class="desc">Supabase PostgreSQL · ${esc(session?.user?.email||'')}</div></div><div></div></div></div><div class="grid"><label class="secondary" style="display:block;text-align:center;cursor:pointer">Importa CSV<input type="file" accept=".csv,text/csv" style="display:none" onchange="importCsv(event)"></label><button class="secondary" onclick="reload()">Ricarica da database</button><button class="secondary danger" onclick="logout()">Esci</button></div>`)}
