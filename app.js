...
function summary(){const groups=groupSummary();const t=totals();return appShell(`<h1>Riepilogo</h1><h2>Riepilogo mese</h2>${monthSelector()}<div class="card"><b>Totale mese</b><div class="metricLine">${metricLine(t.hours,t.amount)}</div><div class="chartWrap"><div class="chartTitle"><span>Andamento mese</span><span>1 → fine mese</span></div>${monthChartSvg()}</div></div><div class="list">${groups.map(r=>`<div class="row summaryRow"><div></div><div><div class="title">${esc(clientName(r.client_id))}</div><div class="desc">${esc(projectName(r.project_id)||'Senza progetto')} · ${esc(r.label)}</div><div class="metricLine">${r.type==='daily_rate_8h'?metricLine(r.hours,r.amount):amountLine(r.label,r.amount)}</div></div><div class="value"></div></div>`).join('')||'<div class="empty">Nessun riepilogo.</div>'}</div><h2>Riepilogo anno</h2>${annualSummaryCard()}`)}
...
function inpsGsCalc(year=currentYear()){
  ...
  const accontoCurrentDue=totalDuePrev;
  ...
  const rata1=accontoCurrentDue*0.4,rata2=accontoCurrentDue*0.6;
...
function render(){if(state.loading){document.getElementById('app').innerHTML=loadingView();return}if(state.view==='resetPassword'){document.getElementById('app').innerHTML=resetPasswordView();return}if(!session){const authMap={register:registerView,forgotPassword:forgotPasswordView};document.getElementById('app').innerHTML=(authMap[state.view]||loginView)();return}let html='';const map={home,newChoice,dailyForm,dailyEdit,monthlyForm,monthlyEdit,manualForm,manualEdit,expenseForm,expenseEdit,timesheet,summary,billing,billingDetail:billingDetailView,settings,clients,projects,activities,clientEdit,projectEdit,activityEdit,expenseCategories,expenseCategoryEdit,invoiceTemplates,invoiceTemplateEdit,appearance,exportTimesheet,tax,taxPayments,taxPaymentEdit};html=(map[state.view]||home)();document.getElementById('app').innerHTML=html}
